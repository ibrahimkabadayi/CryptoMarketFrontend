...
export const priceAlertApi = {
    createAlert: async (request: CreatePriceAlertRequest): Promise<{ message: string }> => {
        const response = await apiClient.post<{ message: string }>('api/pricealerts', request);
        return response.data;
    },

    getActiveAlerts: async (userId: string): Promise<PriceAlertDto[]> => {
        const response = await apiClient.get<PriceAlertDto[]>(`api/pricealerts/user/${userId}/active`);
        return response.data;
    },

    getAllAlerts: async (userId: string): Promise<PriceAlertDto[]> => {
        const response = await apiClient.get<PriceAlertDto[]>(`api/pricealerts/user/${userId}/all`);
        return response.data;
    },

    deactivateAlert: async (alertId: string, userId: string): Promise<void> => {
        await apiClient.put(`api/pricealerts/${alertId}/deactivate`, JSON.stringify(userId), {
